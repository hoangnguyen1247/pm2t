...
Dashboard.init = function() {
    ...
    this.screen.key(["left", "right"], (ch, key) => {
        ...
        if (key.name === "left") {
            if (i == 3) {
                this[boards[0]].style.border.fg = "white";
            } else {
                this[boards[i + 1]].style.border.fg = "white";
            }
        } else {
            if (i == 0) {
                this[boards[3]].style.border.fg = "white";
            } else {
                this[boards[i - 1]].style.border.fg = "white";
            }
        }
...
Dashboard.refresh = function(processes) {
    ...
    // Percent of memory use by one process in all pm2 processes
        ...
        // Status of process
        let status = processes[i].pm2_env.status == "online" ? "{green-fg}" : "{red-fg}";
        status = status + "{bold}" + processes[i].pm2_env.status + "{/}";

        const name = processes[i].pm2_env.name || p.basename(processes[i].pm2_env.pm_exec_path);

        // Line of list
        const item = sprintf("[%2s] %s {|} Mem: {bold}{%s-fg}%3d{/} MB    CPU: {bold}{%s-fg}%2d{/} %s  %s",
        ...
        let j = 0;
        for (const key in proc.pm2_env.axm_monitor) {
            const metric_name = proc.pm2_env.axm_monitor[key].hasOwnProperty("value") ? proc.pm2_env.axm_monitor[key].value : proc.pm2_env.axm_monitor[key];
            const metric_unit = proc.pm2_env.axm_monitor[key].hasOwnProperty("unit") ? proc.pm2_env.axm_monitor[key].unit : null;
...
function timeSince(date) {
    ...
    const seconds = Math.floor(moment().unix() - moment(date).unix());
