...
http.createServer(function (request, response) {
    if (options.basic_auth) {
        ...
        const user = parseBasicAuth(request.headers.authorization);
...
function serveFile(uri, request, response) {
    let file = decodeURIComponent(url.parse(uri || request.url).pathname);

    if (file === "/" || file === "") {
        file = options.homepage;
        request.wantHomepage = true;
    }
    const filePath = path.resolve(options.path + file);
    ...
    const contentType = contentTypes[filePath.split(".").pop().toLowerCase()] || "text/plain";
