...
const postModuleInfos = function (module_name, human_info) {
...
export default function (proc) {
    ...
    if (pm2_env.axm_monitor && Object.keys(pm2_env.axm_monitor).length > 0) {
        ...
        Object.keys(pm2_env.axm_monitor).forEach(function (key) {
            const obj = {};
            const metric_name = pm2_env.axm_monitor[key].hasOwnProperty("value") ? pm2_env.axm_monitor[key].value : pm2_env.axm_monitor[key];
            const metric_unit = pm2_env.axm_monitor[key].hasOwnProperty("unit") ? pm2_env.axm_monitor[key].unit : "";
            const value = `${metric_name} ${metric_unit}`;
            obj[key] = value;
