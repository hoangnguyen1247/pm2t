...
//     try {
//         const dump_raw = require("fs").readFileSync(cst.DUMP_FILE_PATH);
//         const dump = JSON.parse(dump_raw);
//         const apps_dumped = dump.map(proc => proc.name);
//         const apps_running = list
//             .filter(proc => proc.pm2_env.pmx_module != true)
//             .map(proc => proc.name);
//         const diff = apps_dumped.filter(a => !apps_running.includes(a));
//         if (diff.length > 0) {
...
function listModulesAndAppsManaged(list) {
    ...
    list.sort((a, b) => {
        const fieldA = UxHelpers.getNestedProperty(fields[sortField], a);
        const fieldB = UxHelpers.getNestedProperty(fields[sortField], b);
    ...
    list.forEach((l) => {
        ...
        const mode = l.pm2_env.exec_mode;
        const status = l.pm2_env.status;
        let key = l.pm2_env.pm_id;
        key = chalk.bold.cyan(key);

        if (l.pm2_env.axm_options) {
            let is_tracing_enabled = false;
        ...
        if (l.pm2_env.pmx_module == true) {
            ...
            // Module version + PID
            if (!CONDENSED_MODE) {
                const pid = l.pm2_env.axm_options.pid ? l.pm2_env.axm_options.pid : l.pid;
...
function containersListing(sys_infos) {
    ...
    sys_infos.containers.forEach((c) => {
        const cpu = c.stats.cpu_percent;
        const mem = c.stats.mem_percent == 0 ? "0" : c.stats.mem_percent;
        const id = chalk.bold.cyan(proc_id++);
        const state = UxHelpers.colorStatus(c.state);
...
function listHighResourcesProcesses(sys_infos) {
    ...
    const sys_proc_head = ["id", "cmd", "pid", "cpu", "mem", "uid"];

    const sys_proc_table = new Table({
    ...
    sys_infos.processes.cpu_sorted.forEach(proc => {
        const cpu = `${UxHelpers.colorizedMetric(proc.cpu, 40, 70, "%")}`;
        const mem = `${UxHelpers.colorizedMetric(proc.memory, 40, 70, "%")}`;
        const cmd = proc.cmd;
...
function miniMonitBar(sys_infos) {
    let sys_summary_line = `${chalk.bold.cyan("host metrics")} `;
    ...
    if (sys_infos.mem) {
        const perc_mem_usage = (((sys_infos.mem.available) / sys_infos.mem.total) * 100).toFixed(1);
        sys_summary_line += ` | ${chalk.bold("mem free")}: ${UxHelpers.colorizedMetric(perc_mem_usage, 30, 10, "%")} `;
    }
    if (sys_infos.network) {
        const latency = (sys_infos.network.latency).toFixed(1);
        if (latency == -1) {
    ...
    if (CONDENSED_MODE == false) {
        ...
        sys_infos.storage.filesystems.forEach(fs => {
            // disk_nb++;
            const perc_used = ((fs.used / fs.size) * 100).toFixed();
            if (+perc_used > 60) {
...
export default function (list, sys_infos) {
    const pm2_conf = Configuration.getSync("pm2");
