...
function install(PM2, module_filepath, opts, cb) {
    ...
    if (module_filepath.includes("http") === true) {
        const target_file = module_filepath.split("/").pop();
        const target_filepath = path.join(os.tmpdir(), target_file);
...
function retrieveRemote(url, dest, cb) {
    ...
    const wget = spawn("wget", [url, "-O", dest, "-q"], {
        stdio: "inherit",
        env: process.env,
        shell: true
    });
...
function installLocal(PM2, module_filepath, opts, cb) {
    ...
    getModuleName(module_filepath, function (err, module_name) {
        ...
        const install_path = path.join(cst.DEFAULT_MODULE_PATH, module_name);

        require("mkdirp").sync(install_path);

        const install_instance = spawn("tar", ["zxf", module_filepath, "-C", install_path, "--strip-components 1"], {
            stdio: "inherit",
            env: process.env,
            shell: true
        });
...
function deleteModulePath(module_name) {
    // const sanitized = module_name.replace(/\./g, "");
...
function runInstall(PM2, target_path, module_name, opts, cb) {
    const config_file = path.join(target_path, "package.json");
    let conf;
...
function start(PM2, module_name, cb) {
    const module_path = path.join(cst.DEFAULT_MODULE_PATH, module_name);
...
function getModuleName(module_filepath, cb) {
    const tmp_folder = path.join(os.tmpdir(), cst.MODULE_BASEFOLDER);

    const install_instance = spawn("tar", ["zxf", module_filepath, "-C", os.tmpdir(), `${cst.MODULE_BASEFOLDER}/package.json`], {
        stdio: "inherit",
        env: process.env,
        shell: true
    });
    ...
    install_instance.on("close", function (code) {
        try {
            const pkg = JSON.parse(fs.readFileSync(path.join(tmp_folder, "package.json"), "utf8"));
...
function package1(module_path, target_path, cb) {
    const base_folder = path.dirname(module_path);
    const module_folder_name = path.basename(module_path);
    const pkg = require(path.join(module_path, "package.json"));
    const pkg_name = `${module_folder_name}-v${pkg.version.replace(/\./g, "-")}.tar.gz`;
    const target_fullpath = path.join(target_path, pkg_name);

    const cmd = `tar zcf ${target_fullpath} -C ${base_folder} --transform 's,${module_folder_name},module,' ${module_folder_name}`;

    Common.logMod(`Gziping ${module_path} to ${target_fullpath}`);

    const tar = exec(cmd, (err, sto, ste) => {
        if (err) {
...
function publish(PM2, folder, cb) {
    ...
    const current_path = target_folder;
    const module_name = path.basename(current_path);
    const target_path = os.tmpdir();
    ...
    package1(current_path, target_path, (err, res) => {
        ...
        const uri = `${PM2.pm2_configuration.registry}/api/v1/modules`;
