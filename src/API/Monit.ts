...
const Monit = {
    ...
    addProcess: function (proc, i) {
        ...
        const process_name = proc.pm2_env.name || p.basename(proc.pm2_env.pm_exec_path);
        const status = proc.pm2_env.status == "online" ? chalk.green.bold("●") : chalk.red.bold("●");
     ...
     * @return
     */
    drawRatio: function (bar_memory, memory) {
        let scale = 0;
