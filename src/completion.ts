...
export const complete = function complete(name, completer?, cb?) {
    ...
    // if install cmd, add complete script to either ~/.bashrc or ~/.zshrc
    if (env.uninstall) {
        return uninstall(name, completer, function (err, state) {
            console.log(state || err.message);
            if (err) {
                return cb(err);
            }
    ...
    if (!env.words || !env.point || !env.line) {
        return script(name, completer, function (err, content) {
            if (err) {
                return cb(err);
            }
...
export const parseOut = function parseOut(str) {
    const shorts = str.match(/\s-\w+/g);
    const longs = str.match(/\s--\w+/g);
...
function script(name, completer, cb) {
    const p = pth.join(__dirname, "completion.sh");
...
function install(name, completer, cb) {
    const markerIn = "###-begin-" + name + "-completion-###",
        markerOut = "###-end-" + name + "-completion-###"; // eslint-disable-line @typescript-eslint/no-unused-vars
...
function uninstall(name, completer, cb) {
    ...
    readRc(completer, function (err, file) {
        ...
        let part = file.split(markerIn)[1];
        if (!part) {
...
function readRc(completer, cb) {
    const file = "." + process.env.SHELL.match(/\/bin\/(\w+)/)[1] + "rc",
        filepath = pth.join(process.env.HOME, file);
    fs.lstat(filepath, function (err, stats) {
        if (err) {
            return cb(new Error("No " + file + " file. You'll have to run instead: " + completer + " completion >> ~/" + file));
        }
...
function writeRc(content, cb) {
    ...
    fs.lstat(filepath, function (err, stats) {
        if (err) {
            return cb(new Error("No " + file + " file. You'll have to run instead: " + content + " completion >> ~/" + file));
        }
...
//             return cb(err);
//         }
//         const installed = file.match(marker);
//         return cb(!!installed);
//     });
// }
