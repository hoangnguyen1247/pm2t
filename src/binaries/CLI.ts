...
function checkCompletion() {
    return tabtab.complete("pm2", function (err, data) {
        ...
        if (/^--\w?/.test(data.last)) {
            return tabtab.log(commander.options.map(function (data) {
                return data.long;
            }), data);
        }
        if (/^-\w?/.test(data.last)) {
        ...
        const cmdProcess = ["stop", "restart", "scale", "reload", "delete", "reset", "pull", "forward", "backward", "logs", "describe", "desc", "show"];
...
if (_arr.indexOf("--no-daemon") > -1) {
    ...
    pm2.connect(() => {
        ...
        if (process.argv.slice(2)[0] === "completion") {
            ...
            const third = process.argv.slice(3)[0];
...
function patchCommanderArg(cmd) {
    ...
    if ((argsIndex = commander.rawArgs.indexOf("--")) >= 0) {
        const optargs = commander.rawArgs.slice(argsIndex + 1);
